...
func (m *DateTime) SetZone(zone TimeZone) {
	...
	for i := 0; i < length; {
		...
		if c == '%' {
			...
			case 'Y': //四位数的年份表示（0000-9999）
				internal.ItoAW(&theTime, m.year, 4)
			case 'y': //两位数的年份表示（00-99）
				internal.ItoAW(&theTime, m.year, 2)
			case 'm': //月份（01-12）
				internal.ItoAW(&theTime, m.month, 2)
			case 'd': //月内中的一天（0-31）
				internal.ItoAW(&theTime, m.day, 2)
			case 'H': //24小时制小时数（0-23）
				internal.ItoAW(&theTime, m.hour, 2)
			case 'M': //分钟数（00=59）
				internal.ItoAW(&theTime, m.min, 2)
			case 'S': //秒（00-59）
				internal.ItoAW(&theTime, m.sec, 2)
			default:
